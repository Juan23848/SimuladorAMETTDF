...
def cargar_puntajes():
    ...
    return dict(zip(df["CARGO"].str.strip(), df["PUNTAJE 04/2025"])), dict(zip(df["CARGO"].str.strip(), df["PUNTAJE 05/2025"])), df["CARGO"].dropna().unique().tolist()

puntajes_abril, puntajes_mayo, lista_cargos = cargar_puntajes()
...
for i in range(3):
    ...
    with col1:
        c = st.selectbox(f"Cargo u horas #{i+1}", options=[""] + lista_cargos, key=f"cargo_{i}")
    with col2:
...
if st.button("Calcular Comparación"):
    ...
    df_resultado = df_resultado[df_resultado["Concepto"] != "Desglose"]
